...
3. 屏上出现冻结帧 + 十字准星：
   ...
   - 同一画面上 **左键拖拽** 画红框强调（可任意位置、可多个、撤销/恢复）；
     **回车 / 双击 /「完成」** 一次确认发送。
   - **右键 / Esc** 逐级回退：画框中 → 撤框 → 重选 → 退出截图。
...
3. 页面全屏遮罩出现：**左键拖拽**框选 → 选区工具条出现 → 同画面**左键拖拽**
   画红框 → 回车/「完成」发送；**右键/Esc** 逐级回退。
...
- 壳层（engine A）在思灵仓库 `shell/main.mjs` + `shell/screenshot.html`：`desktopCapturer`
  ...
  （选区工具条 + 原图坐标系画红框，合成时按选区裁剪）→ `executeJavaScript`
  派发 `ssid:screenshot` CustomEvent。
